...
$(document).ajaxSend(function (event, request, settings) {
    ...
    $('#revenue_load').show();
    $('#plays_load').show();
    $('#costOfGoods_load').show();
    $('#revenuePerPlay_load').show();
...
$(document).ajaxComplete(function (event, request, settings) {
    ...
    $('#revenue_load').hide();
    $('#plays_load').hide();
    $('#costOfGoods_load').hide();
    $('#revenuePerPlay_load').hide();
...
function renderDateRangePlotChartData(startDate, endDate, cranes) {        
    ...
    var plotObj = $.plot($("#chart_plot_01"), [[new Date().getTime(), 0]], chart_plot_settings); 

    for (var j = 0; j < cranes.length; j++) {
        (function (j) {
            var arr_CostOfGoods = [];
            var arr_TotalRevenue = [];
            var jsonObjects =
            {
                startDate: startDate,
                endDate: endDate,
            ...
            $.ajax({
                type: "POST",
                url: "DashboardData.asmx/GetDateRangePlotChartData",
                cache: false,
                ...
                success: function (msg) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        arr_CostOfGoods.push([new Date(data[i].Date).getTime(), data[i].CostOfGoods]);
                        arr_TotalRevenue.push([new Date(data[i].Date).getTime(), data[i].TotalRevenue]);
                    }
                    dataset.push({ data: arr_TotalRevenue, label: cranes[j] + ' Total Revenue', color: colors[j + 1] });
                    dataset.push({ data: arr_CostOfGoods, label: cranes[j] + ' Cost of Goods', color: colors[j] });
                    plotObj.setData(dataset);
                    plotObj.setupGrid();
                    plotObj.draw();
                },
                error: function (err) {
...
function renderPayoutChartData(dateType, cranes) {
    var dataset = [];
    var ticks = [];
    if (dateType == "day"){
        ticks = [1, "day"];
    ...
    var plotObj = $.plot($("#chart_plot_02"), [[new Date().getTime(), 0]], chart_plot_02_settings);
    for (var j = 0; j < cranes.length; j++) {
        (function (j) {
            var arr_PayoutPercent = [];
            var jsonObjects =
                {
                    dateType: dateType,
                    crane: cranes[j]
            ...
            $.ajax({
                type: "POST",
                url: "DashboardData.asmx/GetPayoutChartData",
                cache: false,
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify(jsonObjects),
                dataType: "json",
                success: function (msg) {
                    var data = msg.d;
                    for (var i = 0; i < data.length; i++) {
                        arr_PayoutPercent.push([new Date(data[i].Date).getTime(), data[i].PayoutPercent]);
                    }
                    dataset.push({ data: arr_PayoutPercent, label: cranes[j], color: colors[j + 1] });
                    plotObj.setData(dataset);
                    plotObj.setupGrid();
                    plotObj.draw();
                },
...
function renderTopTilesData(startDate, endDate, cranes) {
    ...
    $.ajax({
        type: "POST",
        url: "DashboardData.asmx/GetTopTilesData",
        cache: false,
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(jsonObjects),
        dataType: "json",
...
renderDateRangePlotChartData(startDate, endDate, cranes);        
} 

function init_Payout_chart() {

    if (typeof ($.plot) === 'undefined') { return; }

    console.log('init_Payout_chart');
    
    renderPayoutChartData("day", cranes);
} 

function init_top_tiles() {
